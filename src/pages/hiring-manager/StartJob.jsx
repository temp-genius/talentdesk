...
export default function StartJob() {
  ...
  useEffect(() => {
    if (!user || !recruiterId || !jobId) { setLoading(false); return }
    Promise.all([
      supabase.from('jobs')
  ...
  async function handleStart() {
    ...
    if (jraErr || !assignment) {
      setError(jraErr?.message ?? 'Failed to create assignment. Please try again.')
      setSubmitting(false)
      return
    }
    ...
    if (msErr) {
      setError(msErr.message ?? 'Failed to create milestones.')
      setSubmitting(false)
      return
    }

    // Update job status to active + agreed shortlist size
    await supabase
      .from('jobs')
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        <div className="text-center">
          <p className="text-gray-600 mb-4">Invalid link — recruiter or job not specified.</p>
          <Link to="/hiring-manager/dashboard" className="btn-secondary text-sm">Go to Dashboard</Link>
        </div>
      </div>
    )
  }
  ...
  if (!job || !recruiter) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50 px-4">
        <div className="text-center">
          <p className="text-gray-600 mb-4">Could not load the job or recruiter details.</p>
          <Link to="/hiring-manager/dashboard" className="btn-secondary text-sm">Go to Dashboard</Link>
        </div>
      </div>
    )
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-3xl mx-auto px-6 py-10 space-y-6">
          ...
          <div className="card">
            ...
            <div className="space-y-3">
              {MILESTONES_DEF.map(m => (
                <div key={m.number} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                  <div>
        ...
        {/* Shortlist size */}
        <div className="card">
          <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-4">Agreed Shortlist Size</h2>
        ...
        {/* Terms */}
        <div className="card">
          <h2 className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-4">Terms &amp; Conditions</h2>
          <p className="text-xs text-gray-500 mb-4">
