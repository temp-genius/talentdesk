...
/**
 * Approves a recruiter, sends the founding-member email, and logs the action.
 *
 * @param {object} profile  - recruiter_profiles row (must include id, user_id, first_name, last_name;
 *                            optionally email or users.email — will be looked up if missing)
 ...
 */
export async function approveRecruiter(profile, adminUserId) {
  ...
  if (!email) {
    ...
    emailSent = await sendApprovalEmail(email, firstName)
    if (!emailSent) {
